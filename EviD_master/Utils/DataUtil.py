...
def prune_useless_update_from(report: Dict[str, Any]) -> Dict[str, Any]:
    data = deepcopy(report)
    evid_list: List[Dict[str, Any]] = data.get("evid_list") or []

    useful_ids = set()
    for evid in evid_list:
        evid_id = evid.get("evid_id")
        if evid_id is None:
            continue
        obs = evid.get("obligations_status") or {}
        if any(v != "Unknown" for v in obs.values()):
            useful_ids.add(evid_id)

    for evid in evid_list:
        update_from = evid.get("update_from")
        if not update_from:
            continue
        evid["update_from"] = [eid for eid in update_from if eid in useful_ids]

...
from Utils.Gating import (
    make_obligation_gate,
    build_mini_primitives_table,
    updated_obligations,
    check_obligation_gate,
    refill_obligation_gate,
)
...
def main_process(bin_path, cwe_type, direct_flag=False, static_sinks=[]):
    ...
    for iter_func in internal_func_list:
        ...
        # call_find_v7 emits JSON shaped like { func_name: [calls...] }
    ...
    for iter_propagation in propagation_list:
        ...
        update_flag = True
        update_failed_obligations = []

        obligation_gate, mini_primitives = make_obligation_gate(
            call_chains_list,
            func_call_dict,
            local_var_dict,
            global_var_dict,
        ...
        # print(obligation_gate, mini_primitives)

        obligation_gate_str = build_mini_primitives_table(obligation_gate)

        use_obligation_gate = refill_obligation_gate(obligation_gate)
        ...
        vuln_finder = VulnFinder(
            ...
            pcode_list=pcode_list,
            taint_report=reason,
            obligation_gate_str=obligation_gate_str,
            mini_primitives=mini_primitives,
        )
        ...
        # obj_logger.save_log(idx)
        # obj_logger.save_log(vuln_ans,log_type="vuln")

        final_flag = False if vuln_ans.get("state") == "interactive" else True
        while final_flag == False:
            evid_id += 1
            query_type = vuln_ans.get("query_type")
            func_name = vuln_ans.get("func")
            remap_name = demangle_to_symbol_dict.get(func_name)
            var_name = vuln_ans.get("var")

            if query_type == "stack_lvar":
                local_var_list = lookup_all(local_var_dict, func_name, remap_name)
                ...
                func_call_list = lookup_all(func_call_dict, func_name, remap_name)
                ...
                demangle_name_list = [
                    symbol_to_demangle_dict.get(d) for d in func_name_list
                ]
                pass_params = {
                    "func_name": func_name_list,
                    "func_type": func_type_list,
                    "demangle_name": demangle_name_list,
                }

            elif query_type == "fetch_pcode":
                pcode = lookup_all(pcode_dict, func_name, remap_name)
                # pcode = pcode_dict.get(func_name)
                pass_params = {"pcode": pcode}

            elif query_type == "fetch_disasm":
                ehasm = lookup_all(ehasm_dict, func_name, remap_name)
                ...
                if global_var is None:
                    slog.warning(f"[warn] unknown global var: {var_name}")
                    pass_params = {
                        "global_var_type": "UnknownGlobalVar",
                        "global_var_size": -1,
                        "ref_func_list": [],
                        "addr_ea": None,
                        "init_value": None,
                    ...
                    ref_list = global_var.get("references") or []
                    ref_func_list = list(
                        dict.fromkeys(
                            d.get("function_name")
                            for d in ref_list
                    ...
                    pass_params = {
                        ...
                        "init_value": global_var.get("init_value"),
                    }
            ...
            vuln_ans_str = extract_json_string(
                vuln_finder.interact(
                    prior_info=pass_params,
                    type=query_type,
                    update_flag=update_flag,
                    failed_obligations=update_failed_obligations,
                ).read()
            )

            vuln_ans = json.loads(vuln_ans_str)
            # obj_logger.save_log(vuln_ans,log_type="vuln")
            update_obligations_status = vuln_ans.get("obligations_status")
            update_from = vuln_ans.get("update_from") or []
            diff_obligations = updated_obligations(
                obligations_status, update_obligations_status
            )
            update_flag, update_failed_obligations = check_obligation_gate(
                diff_obligations, update_from, pass_cache_list, use_obligation_gate
            )
            if update_flag:
                obligations_status = update_obligations_status
            else:
                gating_flag = True
                slog.warning(str(update_flag) + " : " + str(update_failed_obligations))

            llm_reason.append(vuln_ans.get("reason"))
            llm_obligations_status.append(update_obligations_status)
            real_obligations_status.append(obligations_status)

            final_flag = False if vuln_ans.get("state") == "interactive" else True

        status_record.append({
            "llm_reason": llm_reason,
            "llm_obligations_status": llm_obligations_status,
            "real_obligations_status": real_obligations_status,
            "gating_flag": gating_flag,
            "pass_cache_list": pass_cache_list
        })

        if vuln_ans.get("is_vuln") == True:
            vuln_list.append(
                {

...
#PS D:\IDA91\IDA_PRO> ./ida.exe -A -L"output.log" -S"E:\ida_util\func_dump_v2.py triggerOverflow" E:\vuln_test\test1_junk\main_junk.exe
def main():
    """
    ...
    # 2) 调用旧式的 restore_user_lvar_settings(lvar_uservec_t, ea_t)
    #    注意：第一个参数必须是 lvar_uservec_t*, 而不是 cfunc
    success = ida_hexrays.restore_user_lvar_settings(lvinf, f.start_ea)
    if success:
        # 3) 若成功读取到信息，再用 apply_user_lvar_settings(cfunc, lvar_uservec_t) 把它应用到反编译结果
        ida_hexrays.apply_user_lvar_settings(cfunc, lvinf)

    # 4) 重新构造 c_tree，确保 cfunc.get_lvars() 等能反映最新的变量名
    cfunc.build_c_tree() """

    """ func_ea = idc.get_name_ea_simple("checkPass")  # 假设函数名
    ...
    if tinfo.get_func_details(ftd):
        for i, arg in enumerate(ftd):
            idc.msg(f"Param index: {i}\n")
            idc.msg(f"Param name : {arg.name}\n")
            idc.msg(f"Param type : {arg.type.dstr()}\n") """

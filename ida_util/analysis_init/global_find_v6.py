...
def is_data_candidate(ea):
    ...
    - 在非执行段
    - 且不是明显的代码项
    放宽了“必须是 data item”的限制，以便捕获 .rodata 中 _UNKNOWN 一类对象。
    """
...
def _get_string_size_bytes(ea):
    # 1) item_size —— 只在 size > 1 时信任，避免 1 字节 item 把字符串截断
    try:
...
def deduce_type_and_size(var_ea):
    ...
    # ===== 新增启发式：只读&非执行段，且无有效类型信息时，尝试按 C 字符串处理 =====
                    ...
                    if trimmed:
                        ...
                        for ch in trimmed:
                            if not (32 <= ch < 127):
                                ...
                                break
                        if all_printable:
                            # 例如 .rodata:unk_2008，最终会得到 type = const char[]，size_bytes = 3
...
def deduce_init_value(var_ea, type_str, size_bytes):
    ...
    # 1.b) 只读段里、类型推断为 char[] / const char[] 的情况（比如 .rodata:unk_2008）
    seg = idaapi.getseg(var_ea)

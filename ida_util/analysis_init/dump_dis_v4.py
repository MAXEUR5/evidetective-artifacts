...
新增：
  - 为每个函数额外导出字段 demangle_name（去参名、去返回类型的短名）
...
import json
import os
import subprocess  # ==== 新增: 作为 demangle 兜底 ====
...
# ===== 4.5) ==== 新增: demangle 工具函数 =====
import re  # 新增：放在文件顶部的 import 区

def _normalize_demangled(d: str) -> str:
    """
    规范化 demangle 结果为“仅函数名（含作用域）”，不带返回类型/参数列表/调用约定。
    规则：
      - 先砍掉参数列表 "(...)"
      - 去除常见调用约定标记（__cdecl/__thiscall/__fastcall/__vectorcall 等）
      - 若包含 '::'：从第一个 '::' 往左，截到最近一个空格之后，得到完整作用域+函数名
      - 若不含 '::'：
          * 若包含 'operator' 词：从 'operator' 起截（保留 'operator new'/'operator<<' 等）
          * 否则取最后一个空格后的 token（处理 'int main' → 'main'、'void ~A' → '~A'）
    """
    if not d:
        return d
    ...
    # 2) 去掉常见调用约定（和多余空格）
    #   注意：有些 demangler 会给出 "__cdecl"/"__fastcall" 等
    s = re.sub(r"\b__(?:cdecl|thiscall|fastcall|vectorcall)\b", "", s)
    s = re.sub(r"\s+", " ", s).strip()

    # 3) 若包含命名空间/类限定 '::'：去掉左侧返回类型，只保留作用域+函数名
    if "::" in s:
        first_scope = s.find("::")
        # 第一个 '::' 左边，找最近的空格；从那之后开始截取
        cut = s.rfind(" ", 0, first_scope)
        s = s[0 if cut == -1 else (cut + 1):].strip()
        return s

    # 4) 无作用域：
    #    - operator 系列：从 'operator' 起截（可覆盖 'operator new' / 'operator[]' / 'operator<<'）
    m = re.search(r"\boperator\b", s)
    if m:
        return s[m.start():].strip()

    #    - 其他普通函数，如 'int main'：取最后一个空格后的 token
    if " " in s:
...
def demangle_best_effort(maybe_mangled: str) -> str | None:
    """优先用 IDA 内置 demangler；失败则 c++filt 兜底。返回规范化后的短名。"""
    if not maybe_mangled:
        return None

    # 1) IDA: ida_name.demangle_name
    try:
        # 这些 flag 在不同版本表现略有差异；0 也可让 IDA 自行判别。
        flags = 0
...
def get_func_demangled_name(ea: int) -> str:
    """
    返回函数的 demangle 后短名：
    - 优先从真实符号名（可能是 mangled）做 demangle
    - 若失败，则退回当前可见名（并做一次“规范化”尝试）
    """
    # 当前数据库中的“可见名/原名”
    ...
    if dm:
        return dm
    # 若 demangle 失败，尝试从反编译签名中取作用域路径（保底）
    try:
        cfunc = ida_hexrays.decompile(ea)
        # 反编译首行一般形如：'void __fastcall A::B::f(args)'
        ...
        pass
    # 最终保底：返回原名（可能已是用户重命名的 demangled 名）
...
for func_name in FUNC_NAMES:
    ...
    result[func_name] = {
        ...
        "demangle_name": demangle_name,  # <<<<<< 新字段
    }
...
# 例:
# ./ida -A -L"output.log" -S"/home/workspace/ida_util/analysis_init/dump_dis_v3.py funcs_list=CWE121_Stack_Based_Buffer_Overflow__CWE805_int_alloca_loop_01_bad" /path/to/bin
# ./ida -A -L"output.log" -S"/home/workspace/ida_util/analysis_init/dump_dis_v4.py funcs_list=_ZN59CWE121_Stack_Based_Buffer_Overflow__src_char_declare_cpy_8467CWE121_Stack_Based_Buffer_Overflow__src_char_declare_cpy_84_goodG2BD2Ev" /home/workspace/jc/t1/CWE121_Stack_Based_Buffer_Overflow__src_char_declare_cpy_84-good

...
#  Usage example:
#    ./ida -A -L"output.log" \
#      -S"path_dfs_v4.py start_func=entry end_func=endpoint4" \
#      /path/to/binary
# --------------------------------------------------------------------------
...
def parse_args() -> Tuple[str, str]:
    ...
    if not (start and end):
        idc.msg('Usage: -S"path_dfs_v4.py start_func=<name> end_func=<name>"\n')
        idaapi.qexit(1)
    return start, end
...
def collect_funcs_from_global_table(base_ea: int, max_slots: int = 16) -> Set[int]:
    ...
    This is meant to capture cases like:

        .data.rel.ro: off_3CE8 dq offset sub_136A

    and vtables / small arrays of function pointers.
    """
    res: Set[int] = set()
    ps = pointer_size()
    ...
    for i in range(max_slots):
        ...
        if tgt in (idc.BADADDR, 0):
            continue

        f = ida_funcs.get_func(tgt)
        if not f or f.start_ea != tgt:
            continue
...
# Example:
#   ./ida -A -L"output.log" \
#     -S"/home/user/ida_util/analysis_init/path_dfs_v4.py start_func=main end_func=sub_1320" \
